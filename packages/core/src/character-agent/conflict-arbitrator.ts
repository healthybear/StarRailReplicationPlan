 ...
 *
 * 职责：
 * 1. 检测多角色响应中的冲突（行动冲突、资源争夺、目标冲突）
 * 2. 根据策略裁决冲突（优先级、折中、随机）
 * 3. 生成裁决结果和解决方案
 * 4. 防止死锁（双方均无法行动）
 *
 * 冲突类型：
 * - action_conflict: 行动冲突（两角色行动互斥，如同时开门）
 * - resource_conflict: 资源争夺（争夺同一物品或位置）
 * - goal_conflict: 目标冲突（目标相反，如一方要开门一方要关门）
 * - dialogue_conflict: 对话冲突（信息矛盾，如对同一事件的不同描述）
 *
 * 裁决策略：
 * - priority: 按优先级决定（基于能力值、关系、角色重要性）
 * - compromise: 折中方案（双方各退一步，寻找中间方案）
 * - random: 随机决定（用于测试或无明确优先级时）
 * - first_wins: 先行动者获胜（按响应顺序）
 *
 * 裁决流程：
 * 1. 分析所有角色响应，识别潜在冲突
 * 2. 评估冲突严重程度
 ...
 *
 * 死锁检测：
 * - 如果冲突无法解决（双方均无法行动），标记为死锁
 * - 死锁时需要外部干预（如 GM 介入或重新生成响应）
 *
 * 对应 WBS：P2-CA-01（多 Agent 冲突检测与裁决）
 */
@injectable()
