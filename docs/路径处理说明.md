...
之前在 `session-workspace.ts` 中使用了动态导入：

```typescript
const { EnvLoader } = await import('@star-rail/infrastructure');
```

这导致 `EnvLoader` 的静态状态（`loaded` 标志）可能不一致，造成环境变量无法正确读取。
...
这确保了 `EnvLoader` 的单例状态在整个应用中保持一致。

**相关文件**：

- **[packages/cli/src/ui/session-workspace.ts](../packages/cli/src/ui/session-workspace.ts)** - 改为静态导入 EnvLoader
